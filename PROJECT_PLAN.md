...
> - Architecture & stack decisions: [`project-description/citycode-tech-stack-v2.md`](./project-description/citycode-tech-stack-v2.md) — _treated as the ARCHITECTURE.md for this project_
> - Update this file as work progresses; keep statuses honest.
>
> **Status legend:** ✅ done · 🚧 in progress · ⬜ pending · ⏸️ deferred · ❌ cancelled
>
> Last updated: 2026-09-24 (Phase 7 complete)
...
Phases must complete roughly in order; P4's _visual_ work could start early but depends on P1/P2 outputs and P3's LLM client for summaries. **No new npm dependencies are required in any phase — everything is already installed** (see `package.json`).
...
| Position           | Folder structure only                                | _not_ importance — layout must never shift for emphasis |
...
- [x] `apply.ts` blast radius: transitive closure over _reverse_ import edges from the changed set (BFS on a reverse-adjacency map); all affected, non-changed nodes get the red tint (color reserved for exactly this — Phase 2 encoding contract). `lib/diff/apply.ts`
- [x] **Layout stability (PRD risk §11):** compare renders on the _HEAD graph's_ layout — classification never re-lays. `/api/analyze` with `mode:"prev"` + warm `repoKey` re-serves the static run's exact stored graph+layout JSON (no re-parse, byte-identical layout → zero shift by construction); deleted files render rubble at their old district spot (deterministic hashed slot per fileId), never triggering re-layout
...
- [x] `CompareBar.tsx`: third mode _"About to commit"_ live; BOTH compare buttons disabled with the reason ("not a git repository — compare modes need git history") when the graph has no HEAD sha; summary fetched with `mode` and cached per workdir hash; counts line shows `untracked` and reads "clean — nothing to commit"
...
## Phase 7 — Hardening & Polish ✅ (100%)

**Goal:** Survive real repositories and rough edges — performance fallbacks, error surfaces, and docs that match reality.

**Depends on:** All previous phases. **New modules:** `lib/progress.ts`; fixes spread across existing ones.

### Tasks

- [x] Large-repo guard (PRD risk §11): `lib/parser/buildGraph.ts` exports `MAX_PARSE_FILES = 1500` / `MAX_TOTAL_LOC = 400_000` and gains `BuildGraphOptions { skim?, maxParseFiles?, maxTotalLoc?, onProgress? }` (the number overrides exist for tests only). Above the cutoff without skim → `SkimRequiredError`; `app/api/analyze` catches it and auto-retries the build with `skim: true` → the summarized city (per-file functions and import roads omitted, buildings sized by LOC). Response carries `skim: true`; warnings gain a `"(repo)"` note explaining what was left out. Never a frozen tab
- [x] Pipeline stage feedback: `lib/progress.ts` — the NDJSON stream contract for `/api/analyze`. Requests sending `Accept: application/x-ndjson` get coarse progress lines (`cloning` / `walking` / `parsing "<N> files"` / `layout` / `saving` / `loading-snapshot`) then `{"type":"result",…}` or `{"type":"error","error"}` as the terminal line (streamed runs keep HTTP 200). Requests without the header keep the original plain-JSON response byte-for-byte. UI: `components/ui/ImportForm.tsx` opts in; `components/ui/ProgressStage.tsx` renders the live stage label
- [x] Error surfaces: `buildGraph` maps input-root fs error codes to fixed readable messages (ENOENT → "input folder does not exist", EACCES/EPERM → "permission denied", ENAMETOOLONG → "path is too long", ENOTDIR → "not a folder"); a leading UTF-8 BOM is stripped before parse. Clone/diff/workdir failures already had readable errors (Phases 3/4/5)
- [x] Cross-platform sweep: Windows paths (spaces, backslashes) verified in Phase 1 and re-covered by `tests/crossPlatform.test.ts`; long paths and unreadable roots get the specific messages above
- [x] Replace create-next-app `README.md` with real usage docs (prereqs: Node 24, git, pnpm; env setup; run instructions; usage walkthrough; troubleshooting)
- [x] Update `lib/README.md` to describe the implemented modules (Phase 7 entries: buildGraph guard/options, lib/progress.ts)
- [x] Final pass on this plan + decision notes in the tech-stack doc (§5 note: size cutoffs in code, skim fallback at the analyze route, in-band NDJSON streaming)

### Acceptance

1. ✅ Real large repos fall back gracefully: a 1,920-file repo auto-skims in 821ms into a summarized city (1,100-file repo full-parses in 651ms) — measured, never a frozen tab
2. ✅ Every deliberate failure mode (bad URL, missing key, non-git folder, unreadable/oversized path) shows a readable explanation (unit-tested; streamed error lines verified over HTTP)
3. ✅ `pnpm test` 140/140 green across 15 test files; `pnpm lint`, `tsc`, `pnpm build` clean
4. ✅ The README covers the full clone-to-first-city path (prereqs, env, run, usage); streamed progress verified over HTTP via curl-level POSTs — browser walkthrough not run this session (per user instruction)
...
- Large-repo rendering quality is bounded by the Phase 7 cutoff: repositories above `MAX_PARSE_FILES` (1500 files) / `MAX_TOTAL_LOC` (400k lines) render as summarized skim cities — buildings sized by lines of code, without per-file functions or import roads (compare modes classify changes per file; blast radius is empty in skim because import edges are omitted)

## Currently Working On

**Phase 7 — complete (2026-09-24). Project done.** Hardening & polish fully wired: the large-repo guard (`MAX_PARSE_FILES` 1500 / `MAX_TOTAL_LOC` 400k in `lib/parser/buildGraph.ts`, `SkimRequiredError` auto-retried as a skim build by `/api/analyze` → summarized city with `skim: true` + a "(repo)" warning), stage feedback (`lib/progress.ts` NDJSON stream negotiated by the Accept header; `ImportForm` + `ProgressStage` render live stages), readable input-folder error surfaces (ENOENT/EACCES/ENAMETOOLONG/ENOTDIR mapping + BOM strip), and real docs (README rewritten from the create-next-app boilerplate; `lib/README.md` updated). Evidence: 140/140 tests green across 15 files, lint + tsc + build clean, tokens measured (1,100-file repo full parse 651ms; 1,920-file repo auto-skim 821ms with summarized city), streamed progress verified over HTTP via curl-level POSTs. Browser walkthrough not run this session (per user instruction — API verified at the HTTP level). Next step: none — the project is done; only the stretch parking lot remains.
