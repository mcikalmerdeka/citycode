 ...
 *
 * Pure mapping: a {@link CommitDiff} + the {@link CodeGraph} built at HEAD →
 * a {@link ChangeSet} the renderer consumes. No fs, no git, no LLM.
 *
 * Rules encoded here (PRD §7.3 + the Phase 2 visual contract):
 * - modified → "construction" (orange site + animated crane marker)
 * - added → "fresh" (lime-green fresh construction)
...
import type { ChangeKind, CommitDiff } from "../git/diff";
import type { CodeGraph } from "../types";

/** Visual status of one node in the compare view. */
export type NodeStatus = "construction" | "fresh" | "rubble" | "moved" | "blast";
...
export function applyCommitDiff(graph: CodeGraph, diff: CommitDiff): ChangeSet {
  ...
  for (const file of diff.files) {
    counts[file.kind]++;
    const change: NodeChange = { fileId: file.path, status: statusForKind(file.kind) };
  ...
  // the HEAD graph's reverse edges (fromId imports → toId imported). Deleted
  // files have no node/edges in the HEAD graph, so they open the closure only
  // when a changed id also exists as a node (documented limitation: the
  // true "who imported the deleted file" blast needs the HEAD~1 graph).
...
function statusForKind(kind: ChangeKind): NodeStatus {
  return kind === "modified" ? "construction" : kind === "added" ? "fresh" : kind === "deleted" ? "rubble" : "moved";
}
